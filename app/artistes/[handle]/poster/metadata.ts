...
export async function generateMetadata(props: PosterMetadataProps): Promise<Metadata> {
  ...
  if (artist.userId) {
    ...
    const useDraft = !!(claims && claims.sub === artist.userId && (claims.key ?? 'poster') === 'poster');
    const page = await prisma.artistPage.findUnique({
      where: { userId_key: { userId: artist.userId, key: 'poster' } },
  ...
  const path = `/artistes/${artist.id}/poster`;
  const title = meta?.title || `${artist.name ?? ''} — Affiche`;

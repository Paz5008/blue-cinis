...
export default async function Page(props: { params: PageParams; searchParams?: PageSearchParams }) {
  ...
  const artistData = {
    ...
    artStyle: artist?.artStyle || null,
    location: artist?.location || null,
    instagramUrl: artist?.instagramUrl || null,
    bannerCaption: (artist as any)?.bannerCaption || null,
    socialLinks: (artist as any)?.socialLinks || null,
    artworks: Array.isArray(artworks)
      ? [...artworks, ...LOREM_ARTWORKS].slice(0, 50).map(artwork => ({
        id: artwork.id,
        title: artwork.title,
        ...
        description: (artwork as any).description ?? null,
        medium: (artwork as any).medium ?? null,
        style: (artwork as any).style ?? [],
        mood: (artwork as any).mood ?? [],
        colors: (artwork as any).colors ?? [],
        orientation: (artwork as any).orientation ?? null,
        widthCm: (artwork as any).widthCm ?? null,
        heightCm: (artwork as any).heightCm ?? null,
        isOriginal: (artwork as any).isOriginal ?? true,
        isSigned: (artwork as any).isSigned ?? true,
        status: (artwork as any).status ?? 'available',
      }))
